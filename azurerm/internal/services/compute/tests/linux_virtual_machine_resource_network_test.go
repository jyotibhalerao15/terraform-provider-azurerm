...
resource "azurerm_network_interface" "test" {
 ...
 ip_configuration {
   name                          = "internal"
   subnet_id                     = azurerm_subnet.test.id
   private_ip_address_allocation = %q
 }
}
`, template, data.RandomInteger, allocationType)
}

func testLinuxVirtualMachine_templatePublicIP(data acceptance.TestData, privateStatic, publicStatic bool) string {
	template := testLinuxVirtualMachine_templateBase(data)

	privateAllocationType := allocationType(privateStatic)
...
`, template, data.RandomInteger, publicAllocationType, data.RandomInteger, privateAllocationType)
}
